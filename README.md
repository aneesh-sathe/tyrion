...
- Native Pi Entry Session commands, durable replay, visible Attachment Mode, and explicit handoff
- Production-qualified Pi RPC Worker lifecycle, Skill, control, usage, Result, and containment validation
...
Available Codex app-server, Claude Agent SDK, and production-qualified Pi RPC configurations name an absolute, SHA-256-pinned adapter executable. Tyrion launches each with its own pinned native CLI, brokered provider, and endpoint-restricted OpenShell policy, plus a structured revision-bound Assignment and spend envelope. It validates the exact native model, tool and Skill preparation plus invocation state, lifecycle, usage, and typed Result; forwards journal-first controls; and persists native session telemetry for inspection. Qualified Pi additionally binds the catalog model to the runtime's exact OpenAI provider/model, permits one native Skill per Assignment, clears queued messages before abort, and records authoritative session-total usage. Pi configurations without `settings.production_qualified: true` remain visibly ineligible and cannot route. Launch-time unavailability reuses the approximately-equal routing rule, while durable cleanup survives repeated Control Plane crashes. Git-backed adapters receive only Tyrion-created bundle bindings; Tyrion commits ordinary uncommitted harness edits before independently validating and integrating the candidate. Production reference adapters are in `adapters/`.

Launch the native Pi Entry adapter with `tyrion --socket PATH pi`. Tyrion issues a single-use launch credential, materializes the pinned extension privately, and starts Pi with explicit extension loading. The extension negotiates all supported capabilities by default, displays `Tyrion: Full`, `Tyrion: Limited`, `Tyrion: Observer`, or `Tyrion: Disconnected` persistently, and exposes native commands for proposal, acceptance, inspection, replay, steering, interruption, retry, Approval Gate inspection, pause, resume, cancellation, downgrade, and takeover. See [Pi Entry and Worker adapters](docs/pi-adapter.md).
